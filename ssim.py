...
def ssim(img1, img2, cs_map=False):
    ...
    C1 = (K1*L)**2
    C2 = (K2*L)**2
    mu1 = signal.fftconvolve(window, img1, mode='full')
    mu2 = signal.fftconvolve(window, img2, mode='full')
    mu1_sq = mu1*mu1
    mu2_sq = mu2*mu2
    mu1_mu2 = mu1*mu2
    sigma1_sq = signal.fftconvolve(window, img1*img1, mode='full') - mu1_sq
    sigma2_sq = signal.fftconvolve(window, img2*img2, mode='full') - mu2_sq
    sigma12 = signal.fftconvolve(window, img1*img2, mode='full') - mu1_mu2
    if cs_map:
